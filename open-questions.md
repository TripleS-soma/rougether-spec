...
- **`todos.external_id` collation**: 현재 utf8mb4 기본 collation이라 unique 중복 판정의 대소문자 무시·PAD 동작이 DB 설정에 따라 다르다. Google 이벤트 id는 소문자라 실사용 문제는 없지만, 대소문자를 구분하는 id 체계(MS Graph 등)를 출처로 받으려면 `external_id`를 `utf8mb4_bin`으로 바꿔야 한다. 앞뒤 공백은 서버가 trim해서 PAD 차이에는 기대지 않는다. (서버)
