...
- **조정 추천 룰 수치 재검증**: 근거 창 3주 · 실패 요일 제외(나머지 요일 완료율 ≥ 50%) · 빈도 축소(2주 연속 < 40%, 상위 최대 3요일) · 활성 상한 3건 · 재추천 쿨다운 14일 · 만료 7일은 초안 기본값 — 운영 데이터(수락/무시/만료율, 수락 후 완료율 변화)로 조정한다. `ADJUST_TIME`(수행 시각 조정) 룰은 완료 시각 데이터 기반 설계 후 후속. (서버)
