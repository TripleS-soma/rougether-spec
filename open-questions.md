...
- **LLM 개인정보 3자 전송**: 회고 생성 시 닉네임·소개글(`bio`)·선택 목표·루틴 제목·카테고리명이 외부 LLM 제공자(OpenAI 호환 API, 현재 NVIDIA NIM)로 전송된다(이메일·집·타인 정보는 제외). 개인정보 처리방침·약관에 3자 제공 고지가 필요한지, 루틴 제목 같은 자유 텍스트를 마스킹/요약해 보낼지 미정.
