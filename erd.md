...
출처: [ERDCloud — 루게더 mvp (최종)](https://www.erdcloud.com/d/Qn9GqwdWnsqsiQQpi) · 총 **51 table**(`attendance_events`·`attendance_check_ins`·`routine_recommendations`·`weekly_reports`·`daily_incomplete_digests`·`daily_incomplete_digest_targets` 포함 — ERDCloud 정본 반영 필요).
...
- **users**: id* | nickname VARCHAR(30)? | bio VARCHAR(100)? | email VARCHAR(255)? | profile_image_key VARCHAR(255)? | last_accessed_at TIMESTAMP? | is_bot BOOLEAN | bot_key VARCHAR(40)? | created_at | updated_at | deleted_at? | unique (bot_key)
  ...
  - `is_bot=true`는 서버가 시드한 로그인 불가 동거 봇이며 관측·보상·알림·배치·회고 대상에서 제외한다. `bot_key`는 봇 프로필 카탈로그의 멱등 키이고 일반 사용자는 null이다.
