...
- **routine_logs**: id* | routine_id→routines | routine_date DATE | status VARCHAR(30) | completed_at TIMESTAMP? | reward_currency_type VARCHAR(30)? | reward_amount INT | created_at
  - `status`: `PENDING`(미수행)/`COMPLETED`(완료)/`FAILED`(실패 — 하루 마감 배치가 전날 미수행 루틴에 기록, `completed_at` null·`reward_amount` 0). 늦은(과거) 완료 시 `FAILED` row는 `COMPLETED`로 전이(UPDATE)된다. 같은 루틴·날짜 로그는 `UNIQUE(routine_id, routine_date)`로 1건.
