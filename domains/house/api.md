...
- **앱은 여기에 더해 삭제를 실행한 본인의 연동 루틴을 지운다**(2026-08-19 확정). 서버 계약(연동만 해제)과 다른 앱 측 동작이며, 미션을 없애기로 한 사람에게는 그 루틴도 남길 이유가 없다는 판단이다. **다른 구성원의 루틴은 건드리지 않는다** — 그들에게는 이미 자기 습관이므로 연동만 끊고 개인 루틴으로 남긴다. 삭제 확인 문구가 이 둘을 구분해 말한다.
- **집 나가기에서도 앱은 그 집 연동 카테고리를 루틴·투두째 삭제한다**(2026-08-19 확정, mobile#908). 위와 같은 결의 판단이다 — 집을 떠나기로 한 사람에게 그 집 이름의 카테고리와 루틴을 남길 이유가 없다고 봤다. 서버 계약(연동만 해제)과 다른 앱 측 동작이고 **되돌릴 수 없으므로, 나가기 확인 문구가 이 사실을 명시한다**(mobile#911). 연동만 푸는 경로(`DELETE /categories/{id}/house-link`)는 서버에 있으나 앱이 쓰지 않는다.
