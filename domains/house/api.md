...
- res: **204 No Content** (2026-08-16 구현 확인). 갱신된 목록을 돌려주지 않으므로 클라이언트가 자기 상태를 직접 반영한다.
- 저장: 배열 index를 `house_members.sort_order`에 0부터 기록
- **전량 전송 계약**: 부분 목록을 허용하지 않는다. 빠진 집을 서버가 임의 위치로 밀어 넣으면 사용자가 지정하지 않은 순서가 조용히 생긴다.
- 예외: **부분 목록·중복 id·내가 구성원이 아닌 집 id가 모두 `HOUSE_ORDER_INVALID`(400)** 하나로 처리된다. 빈 배열은 `VALIDATION_FAILED`(400).
  - 최초 설계는 "요청이 틀렸다(400)"와 "네가 본 목록이 낡았다(409 `HOUSE_ORDER_STALE`)"를 나누려 했으나 **구현은 400 하나로 갔고, 그대로 둔다** — 클라이언트는 항상 자기가 아는 전량을 보내므로 부분·중복·남의 집을 보낼 방법이 없다. 즉 이 400은 실질적으로 "낡은 목록" 신호 하나뿐이고, 두 코드를 나눌 실익이 없다.
  - 클라이언트는 이 400을 받으면 되돌리지 말고 `GET /me/houses`로 다시 받아야 한다. 되돌리기만 하면 낡은 목록이 남아 재시도가 계속 실패한다.
- table: `house_members`
...
- DB 및 일반 신규 집의 기본값은 `false`입니다. `enabled=true`이면서 공개인 집만 온보딩 자동 매칭 대상입니다. 비공개 전환은 즉시 매칭에서 제외하며, 공개 여부 자체는 기존 집 설정 API로 변경합니다. 이번 기존 공개 집 초기 적용은 아래 운영 정책을 따릅니다.
...
설정 변경 요청 예시 — 공개 집의 자동 입주 허용:

```http
PUT /api/v1/houses/42/auto-join
...
응답 필드는 `houseId=42`, `enabled=true`입니다. 중지하려면 같은 API에 `{"enabled":false}`를 보냅니다. 프론트의 방장 설정 화면은 GET의 `enabled`로 현재 값을 표시하고 PUT으로 변경합니다. 이 API는 비공개 집을 공개로 전환하지 않습니다.

### 기존 공개 집 초기 적용 (2026-09-13)

- 프론트에 자동 입주 허용 표시·설정이 아직 연결되지 않은 초기 상태를 고려하여, 운영자 결정으로 **적용 시점의 미삭제 공개 집 전체**에 `onboarding_auto_join_enabled=true`를 한 번 적용합니다. 비공개 집과 삭제된 집은 변경하지 않습니다.
- 적용 결과(2026-09-13): 기존 미삭제 공개 집 10개를 허용 상태로 변경하고 별도 DB 조회로 확인했습니다. 비공개·삭제 집 77개는 변경하지 않았습니다. 이 수치는 적용 당시 기록이며 현재 집 수를 보장하지 않습니다.
- 이는 기존 데이터의 일회성 초기 설정입니다. 이후 생성되는 일반 공개 집의 기본값은 `false`이며, `AUTO_JOIN`의 `NO_MATCH` 개인집 전환 또는 소유자의 설정 API 호출로 허용할 수 있습니다.
- 이후 소유자가 `false`로 변경한 값을 주기적으로 다시 켜지 않습니다. 정원·실사용자·소유자·사용자별 가입 이력 조건은 [온보딩 집 선택 API](../member/api.md#온보딩-집-선택)대로 계속 검사하므로 허용된 집이 항상 매칭되는 것은 아닙니다.
- 프론트는 온보딩 `좋아요`에 `PUT /api/v1/onboarding/house`의 `AUTO_JOIN`, `괜찮아요`에 `PERSONAL`을 연결하고, 별도로 방장에게 위 설정 조회·변경 기능을 제공해야 합니다.
