...
- **회원가입 기본 집**: 회원가입(최초 소셜 로그인·dev-login 신규 가입) 트랜잭션에서 지갑 발급과 함께 사용자 소유의 기본 공동집을 1회 생성한다. 이름 `나의 집`, **비공개(`is_public=false`)**, 정원 4명, 게시 승인 커버 manifest 첫 항목을 적용하고, **집 목표는 비어 있는 채로** 만든다. 가입 시점엔 닉네임이 없어 이름 확정은 닉네임 저장(`PUT /api/v1/me`)이 담당한다 — 이름이 아직 기본값 `나의 집`인 소유 집을 `{닉네임}의 집`으로 개명하며, 직접 지은 이름은 불변이고 한 번 개명되면 이후 닉네임 변경엔 따라가지 않는다(#350). 공개 전환은 집 설정 `isPublic` 토글로 소유자가 직접 한다. 백필(V62): 실사용자(봇 제외) ACTIVE 구성원 1명 이하인 혼자 있는 집은 전부 비공개 처리했고, 기본 이름이면 소유자 닉네임으로 개명했다(닉네임 없으면 유지 — 탐색 `나의 집` 도배 정리). 집 생성 실패는 가입 실패다(집 없는 계정을 만들지 않음). 서버의 동거 봇 규칙에 따라 초기 구성원이 함께 들어갈 수 있다. 집 목표는 `PUT /api/v1/onboarding/goals` 저장 시 "내가 OWNER이고 집 목표가 비어 있는 집"에 대표 목표 우선 + 나머지 목표 마스터 정렬순으로 최대 3개를 1회 채우며, 이후 사용자 목표 변경은 집 목표에 반영하지 않는다. 가입 경로에서는 기본 집을 나갔거나 해체된 뒤 재생성하지 않고, 기존 계정에 소급 생성하지 않는다. 별도 온보딩 집 선택 API에서 시작할 개인집이 없으면 생성하는 예외는 아래 온보딩 자동 합류 계약을 따른다. (`house`, `house_members`, `house_goals`)
...
## 온보딩 자동 합류 (2026-09-13)

- 자동 입주 허용 상태인 공개 집에 신규 사용자를 즉시 합류시킵니다. 일반 신규 집의 기본값은 비허용이며, 활성 방장이 [자동 입주 설정 API](api.md#온보딩-자동-입주-허용-설정)로 조회·변경합니다.
- 2026-09-13 초기 운영 정책으로 적용 시점의 미삭제 공개 집 전체를 한 번 허용합니다. 비공개·삭제 집은 제외하고, 이후 소유자의 비허용 변경은 유지합니다. 프론트 설정 연결 전의 일회성 조치이며 신규 집의 기본값은 변경하지 않습니다.
- 사용자 `AUTO_JOIN` 선택에서 매칭 대상이 없으면 개인집을 공개·자동 입주 허용으로 전환하여 다음 사용자의 매칭 대상이 되게 합니다. `PERSONAL` 선택은 비공개 개인집을 유지합니다. 다른 집에 합류해도 개인집은 유지합니다.
- 상세 선택·재시도 계약은 [회원 API](../member/api.md)의 온보딩 집 선택을 따릅니다. 회원가입 시 기본 집 지급 정책은 유지합니다.
