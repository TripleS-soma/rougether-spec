...
- **응원**: 같은 집 사람이 오늘(KST) 루틴·투두를 완료하면, 그 완료 뒤 **30~90분 창**에서 봇이 1회 응원한다(완료마다 30~80분 결정론적 지연 + 10분 틱). 같은 완료에 두 번 보내지 않고, 봇 1명이 같은 사람에게 보내는 응원은 **하루 2회**까지(봇 기준 — 봇이 2명인 집이면 사람 1명이 하루 최대 4건 받을 수 있음). 타입은 `great`/`support`/`best` 중 결정론적 랜덤. 기존 응원 한도(같은 보낸이→대상·타입 5회/일)와 알림(`FRIEND_CHEER`) 규칙은 그대로다.
