...
- **미션 분모**: `DAILY_MEMBER_RATE` 달성률의 분모(`current_member_count`)에 봇도 포함한다 — 봇을 일반 멤버와 동일하게 취급한다. 봇의 미션 기여·응원·방명록 규칙은 아래 "동거 봇 활동".

## 동거 봇 활동

동거 봇이 집 안에서 하는 행동 규칙. 스케줄러 공통 규칙(10분 틱·활동 창·사람 있는 집만·결정론적 판정·쉬는 날·실패 격리)은 [member/features.md](../member/features.md) "동거 봇 계정"의 활동 스케줄러 항목이 정본이고, 여기서는 집 도메인 행동만 적는다. 쓰기는 전부 사람이 쓰는 기존 서비스 경로(미션 기여·응원·방명록 작성)를 그대로 타며, 신규 API 는 없다.

- **단체 미션 기여**: 봇이 속한 집의 진행 중(ACTIVE·기간 안) 미션마다 하루 1회 **70% 확률**로 수동 기여 경로(`POST .../contribute` 와 동일)로 기여한다. 일반 멤버와 동일 취급 — WEEKLY 기여 합·DAILY 달성률(분자·분모 모두)·집 성장 포인트에 그대로 반영되고, 하루 1회 이력(`house_mission_daily_contributions`)도 공유한다. 봇 루틴을 미션에 연동하지는 않는다(자동 기여 없음). **claim 은 봇이 하지 않는다**(사람 몫). 쉬는 날에는 기여하지 않는다.
- **응원**: 같은 집 사람이 오늘(KST) 루틴·투두를 완료하면, 그 완료 뒤 **30~90분 창**에서 봇이 1회 응원한다(완료마다 30~80분 결정론적 지연 + 10분 틱). 같은 완료에는 집의 봇 전체를 통틀어 한 번만 보내고, 한 사람이 하루에 받는 봇 응원은 **사람 기준 2회**까지다(집의 봇 전체 합산 — 봇이 2명이어도 하루 최대 2건). 타입은 `great`/`support`/`best` 중 결정론적 랜덤. 기존 응원 한도(같은 보낸이→대상·타입 5회/일)와 알림(`FRIEND_CHEER`) 규칙은 그대로다.
- **방명록**: 집당 **주 1~2회** — 월요일은 100%, 목요일은 50%(기대 1.5회/주). 해당일에는 집·날짜로 정한 봇 1명이 사람 1명 방에 템플릿 30개 중 하나(닉네임·요일·완료 개수 치환)로 **하루 1회** 작성하며, 작성 시각은 그 봇의 활동 창 안 목표 틱 이후 첫 틱이다. 치환하는 "오늘 완료 개수"는 방 주인의 **HOUSE·PUBLIC 카테고리 완료만** 센다(비공개·친구공개·미분류 완료 수는 문구에 노출하지 않음 — 집 구성원 완료 열람 범위와 동일). 금칙어 검사·길이 제한은 방명록 작성 API 와 동일하게 적용되고, 목록 응답에는 `authorBot=true` 로 내려간다.
