...
- **연동 루틴 정리(앱)**: 미션 삭제는 소유자 전용이라, 구성원에게는 자기가 만든 연동 루틴을 되돌릴 길이 미션 화면에 없었다(2026-08-19). 연동된 구성원에게는 카드의 `루틴 연동됨`/`기여함` 배지를 눌러 **본인 연동 루틴만 삭제**하는 경로를 준다 — 미션 자체는 그대로다. 소유자의 휴지통과 **결과가 다르므로 아이콘을 공유하지 않고**, 확인 문구가 무엇이 지워지는지 말한다. (앱 전용 동작, 서버 API 는 루틴 삭제 `DELETE /routines/{id}` 재사용)
