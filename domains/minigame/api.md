...
게임 코드는 `room-runner`, `cat-stairs`, `cat-merge`를 유지한다. 목록·시작·랭킹은 선택 쿼리 `rulesVersion`을 받으며 지원 값은 1과 2다. 생략하면 기존 앱과 같은 규칙 1을 사용하고, 새 앱은 난이도를 높인 규칙 2를 명시한다. 지원하지 않는 버전은 오류이며 다른 버전으로 자동 대체하지 않는다.

완료 API는 버전 쿼리를 받지 않는다. 서버에 저장된 `runId`의 규칙 버전으로 검증·최고점·순위를 계산하므로 클라이언트가 완료 시점에 버전을 바꿀 수 없다.
...
| `bestScore` | 이번 결과까지 반영한 해당 게임·세션 규칙 버전의 본인 최고점 |
| `personalBest` | 해당 게임·세션 규칙 버전의 최초 기록이거나 같은 버전 최고점보다 높은지 여부 |
| `rank` | 같은 게임·세션 규칙 버전의 `bestScore`를 기준으로 한 최초 완료 당시의 순위 |

같은 완료 입력의 재전송은 만료 이후에도 위 최초 응답을 그대로 반환한다. 최고점·현재 순위가 필요하면 같은 `rulesVersion`으로 랭킹 API를 다시 조회한다. 유효한 다른 입력을 같은 세션에 제출하면 409 `MINIGAME_RUN_ALREADY_FINISHED`다.

## 전체 회원 랭킹

`GET /api/v1/minigames/{gameCode}/leaderboard?rulesVersion=2` → 200. 페이지·기간·집 필터 없음. 버전 생략 시 규칙 1 랭킹이다.
...
- `items`: 해당 게임·요청 규칙 버전의 개인 최고점 상위 50명. 공동 순위여도 최대 50명이다.
- `myEntry`: 본인 최고점과 현재 순위. `items` 포함 여부와 무관하며, 요청 버전의 완료 기록이 없으면 null이다.
- `totalPlayers`: 해당 게임·요청 규칙 버전에 최고점이 있는 미탈퇴 일반 회원 수. 봇은 제외한다.
- 해당 버전 참가자가 없으면 `{"items":[],"myEntry":null,"totalPlayers":0}`이다. 다른 버전의 기록을 합산하거나 대체하지 않는다.
...
| 400 | `MINIGAME_RULES_VERSION_NOT_SUPPORTED` | 목록·시작·랭킹에서 지원하지 않는 규칙 버전 요청 |
