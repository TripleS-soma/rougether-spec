...
- 초기 잔액: 가입 시 통화별 지갑이 발급되며 **코인 500·다이아 0**으로 시작. 온보딩(튜토리얼)에서 가구 뽑기 단챠(코인 250) 1회를 체험시키고 250이 남게 하는 값. 지급은 가입 트랜잭션의 지갑 신규 발급에만 묶여 1회로 보장(`(user_id, currency_type)` UNIQUE).
