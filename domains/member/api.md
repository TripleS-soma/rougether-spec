...
- 최초 로그인은 자동 가입: `oauth_accounts`에 (provider, provider_user_id)가 없으면 `users`와 통화별 지갑, 사용자 소유 기본 집(`나의 집`, 집 목표 비어 있음 — [house/features.md](../house/features.md) 회원가입 기본 집)을 같은 트랜잭션에서 생성해 연동하고 `isNewUser=true`. 기본 집 생성 실패는 가입 실패. 지갑 초기 잔액은 코인 100·다이아 0(정책은 재화 도메인 소유 → [shop/api.md](../shop/api.md)). 닉네임은 가입 시 비우고 온보딩에서 채운다. 이메일은 카카오가 제공/동의한 경우 가입 시에만 저장(미제공 시 null, 재로그인 시 갱신 안 함).
...
- **같은 이메일의 타 provider 계정 안내(409)** — 카카오·구글·애플 공통. 소셜 **최초 가입**(해당 provider·`provider_user_id` 연동이 없음)인데, provider가 **인증한 이메일**(카카오 `is_email_valid && is_email_verified`, 구글·애플 `email_verified` 클레임)과 같은 이메일(대소문자 무시)로 가입된 **활성**(미탈퇴·비봇) 계정이 **다른 provider**로 있으면, 계정을 만들지 않고 409 `AUTH_EMAIL_LINKED_TO_OTHER_PROVIDER`를 반환한다. 응답 `message`는 "이 이메일은 애플 로그인으로 가입되어 있어요."(여러 개면 "애플·구글"처럼 이어 붙이며 순서는 enum 순 KAKAO→APPLE→GOOGLE 고정), `details.providers`는 같은 순서의 provider 목록(예: `["APPLE"]`). 에러 응답의 `details` 필드는 공통 [api.md](../../api.md) "에러 응답" 참고.
  - 검사 시점은 provider 토큰 검증 **뒤**·가입 트랜잭션 **앞**이다(빈 계정을 만들지 않음). 애플은 1회용 `authorizationCode` 교환 **앞**에서 검사하므로 409를 받아도 코드가 소모되지 않는다 — 재요청은 반드시 **직전과 같은 identityToken·authorizationCode를 5분 안에** 다시 보내야 하며, Sign in with Apple을 새로 태우면 이메일 클레임이 빠져 새 계정의 이메일이 비게 된다.
  - `allowNewAccount`(boolean, 생략 시 false)를 `true`로 재요청하면 안내 없이 새 계정을 만든다(기존 자동 가입 동작). 이미 연동된 회원의 재로그인, 이메일 없음/미인증, 같은 이메일 계정이 탈퇴 계정뿐인 경우, 같은 provider 계정만 있는 경우는 안내 없이 기존대로 동작한다.
...
- `PUT /api/v1/onboarding/goals`는 가입 때 만들어진 기본 집(내가 OWNER이고 `house_goals`가 비어 있는 집)의 집 목표가 비어 있으면 이 저장에서 대표 목표 우선 + 나머지 `sortOrder` 순으로 최대 3개를 1회 채운다. 이미 채워졌으면 바꾸지 않는다(이후 목표 변경은 집 목표에 반영 안 함). 기본 집을 나가 소유권이 넘어갔으면 채우지 않는다. 집 생성 자체는 회원가입 트랜잭션에서 끝나므로 온보딩 호출 순서·경로에 영향받지 않는다. (`house_goals`)
...
| --- | --- | --- |
| `GET /api/v1/onboarding/house` | 본인의 집 선택 결과 조회 | 미선택: `completed=false`, 나머지 필드 null. 선택 후: 아래 PUT 응답과 동일 |
...
- 개인집 재사용 대상은 본인 소유·본인 ACTIVE OWNER·비공개·실사용자 1명인 미삭제 집이며, 여러 개면 ID 오름차순 첫 집입니다. 동거 봇은 실사용자 수에서 제외합니다. 대상이 없으면 기존 기본 집 생성 정책으로 생성하고 현재 선택 목표를 최대 3개 연결합니다.
- 자동 매칭은 공개·자동 입주 허용·미삭제 집 중 실사용자가 1명 이상이고 실사용자 자리가 있는 집을 ID 오름차순으로 탐색합니다. 본인 소유 집, 봇/탈퇴한 소유자의 집, 가입·탈퇴·강퇴 멤버십 이력이나 입주 신청 이력이 있는 집은 제외합니다. 봇이 정원을 채우고 있으면 기존 자리 양보 정책을 적용합니다.
- 집 락 이후 조건을 재검증하며, 후보가 만석·비공개·비허용으로 바뀌었으면 다음 후보를 시도합니다. 가입·정원 증가·입주 알림·선택 결과 저장은 같은 트랜잭션입니다.
- 같은 `choice` 재요청은 최초 결과를 반환합니다. 다른 `choice`는 409 `ONBOARDING_HOUSE_ALREADY_SELECTED`입니다. 이후 탈퇴·강퇴·해체되어도 재가입이나 재매칭하지 않습니다. 응답의 ID는 선택 당시 결과이며 현재 가입 여부는 내 집 목록으로 확인합니다.
- 잘못된 enum·누락·null은 400, 인증 누락은 401입니다. 기존 온보딩 요약의 목표·캐릭터 기준 `completed`는 바꾸지 않습니다.

예시 — 매칭할 집이 없는 첫 사용자의 `좋아요`:

```json
{"completed":true,"choice":"AUTO_JOIN","result":"NO_MATCH","houseId":42,"membershipId":73}
```

이 경우 42번 집은 공개·자동 입주 허용 상태가 되어 다음 신규 사용자의 매칭 후보에 포함됩니다.
