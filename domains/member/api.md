...
| method · path | 목적 | 핵심 필드 | 관련 table |
| --- | --- | --- | --- |
| `DELETE /api/v1/me` | 회원탈퇴 — soft delete + 소셜 로그인 연동 해제(revoke) | res: 204. 이미 탈퇴한 사용자는 404 `USER_NOT_FOUND` | `users`, `oauth_accounts`, `refresh_tokens`, `user_device_token` |

- 탈퇴는 단일 트랜잭션으로 처리한다: `users.deleted_at` 세팅(soft delete — hard delete 아님) + 보유 중인 active `refresh_tokens` 전량 폐기(`revoked_at`) + `oauth_accounts` row 삭제 + FCM 토큰(`user_device_token`) 전량 삭제(탈퇴자에게 push가 가지 않도록). 알림 내역(`notifications`)·설정은 건드리지 않는다. 소유권은 인증된 본인(`user_id`) guard, 별도 파라미터 없음.
...
- 탈퇴 계정의 잔여 토큰 차단: refresh 회전과 dev-login에서 `users.deleted_at`을 확인해 거부한다(각각 `REFRESH_TOKEN_INVALID`, `USER_NOT_FOUND` — 신규 에러코드 없음). 이미 발급된 access token은 만료(30분) 전까지 유효하다(stateless JWT의 알려진 한계, logout과 동일).
- 루틴·투두·카테고리 등은 연쇄 soft delete하지 않는다 — 재가입하면 새 userId로 시작하므로 옛 계정 데이터는 새 계정에 노출되지 않는다.
- house/room·캐릭터·아이템·재화 도메인의 탈퇴 회원 처리(소유 중인 house의 탈퇴 허용·소유권, 길드북 등 닉네임 노출, `user_characters`/`user_items`/`user_wallets` 잔여 데이터)는 이 도메인에서 확정하지 않는다(해당 도메인 dependency → [open-questions.md](../../open-questions.md)).
