...
| 회원탈퇴 | soft delete(`deleted_at`) + refresh token 전량 폐기 + oauth 연동 삭제 + FCM 토큰 삭제를 단일 트랜잭션으로, provider 연동 해제(카카오 unlink·애플 revoke)는 커밋 후 best-effort. 재가입은 즉시 허용 — 재로그인 = 신규 가입(새 user, 옛 데이터 미복원). | `users.deleted_at`, `refresh_tokens`, `oauth_accounts`, `user_device_token` |
...
- 탈퇴 상세 계약(revoke 방식·잔여 토큰 차단·타 도메인 dependency)은 [api.md](api.md) "회원탈퇴" 참고.
