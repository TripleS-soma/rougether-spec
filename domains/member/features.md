...
- **활성화 스위치**: `rougether.bots.enabled`(기본 **false**). 꺼져 있으면 시드와 활동 스케줄러(아래) 모두 돌지 않는다(테스트 컨텍스트도 false) — 시드와 활동을 따로 켜는 스위치는 없다. 봇 격리·활동 규칙이 갖춰진 배포 환경에서만 켠다.
...
- **가구 시드**: 게시(published) 카탈로그의 positioned 가구·소품 6~8개를 보유로 지급하고, 개인 방은 `FREE_V1` 프리셋 1번 좌표로 배치한다(프리셋 2·3은 활동 스케줄러의 주간 레이아웃 순환에서 사용 → [room/features.md](../room/features.md)).
...
- **표시 정책**: 봇 닉네임에 "(봇)" 같은 접미를 붙이지 않는다. 봇 여부는 집 구성원 목록 응답의 `bot`, 방명록 목록 응답의 `authorBot`(boolean)으로 내려가고 프론트는 배지만 표시한다. 방 방문·응원 응답에는 봇 필드가 없다(구성원 목록으로 판별) → [house/api.md](../house/api.md).
- **거주 규칙(집 도메인)**: 온보딩 기본 집에 봇 2명 자동 입주(사용자가 만든 집에는 안 들어감), 만석에 사람이 오면 가장 나중에 들어온 봇이 자리 양보, 봇에게 소유권 양도 불가·회원탈퇴 승계 후보 제외, 사람이 0명이 되면 봇 전원 LEFT + 집 해체, 방장 강퇴 가능 → [house/features.md](../house/features.md) "동거 봇 거주".
- **활동 스케줄러**: 봇의 행동(루틴 완료·단체 미션 기여·응원·방명록·거미줄 청소·방 레이아웃 순환)은 user-api 안의 내부 스케줄러가 **10분 틱(KST)** 마다 판정·실행한다. LLM 없음, 신규 API 없음 — 쓰기는 전부 사람이 쓰는 기존 서비스 경로를 그대로 타므로 보상·상한·한도·금칙어·락 규칙이 사람과 동일하게 적용된다. 공통 규칙은 여기, 행동별 규칙은 각 도메인 문서에 둔다.
  - **처리 대상**: 프로필 카탈로그에 있는 봇(카탈로그에 없는 `bot_key`는 건너뜀) 중 지금 시각이 자기 활동 창(MORNING 06–09·12–13 / EVENING 18–23 / SPREAD 08–22) 안이고, **사람 ACTIVE 구성원이 1명 이상인 집**에 ACTIVE 로 속한 봇만. 사람이 없는 집(봇뿐·빈 집)에서는 아무 행동도 하지 않고, 활동 창 밖에서도 아무 것도 하지 않는다.
  - **결정론적 판정**: 모든 확률·시각 판정은 (봇, KST 날짜, 대상, 틱)에서 만든 결정론적 난수로 뽑는다 — 같은 입력이면 같은 결정(재현 가능), 서버 재시작에도 흔들리지 않는다.
  - **쉬는 날**: 봇·날짜 단위로 하루 15% 확률로 쉬는 날이 되며, 그날은 루틴 완료·미션 기여를 전부 건너뛴다(응원·방명록·거미줄·레이아웃은 쉬는 날과 무관).
  - **틱을 놓쳐도 따라잡음**: 하루 확률로 결정되는 행동(루틴 완료·미션 기여·방명록)은 활동 창 안의 목표 틱을 하나 뽑고 "그 틱 이후 첫 틱"에 실행하므로, 서버가 몇 틱 멈춰도 그날 안에 수행된다. 과거 날짜 소급은 없다.
  - **실패 격리**: 봇 1명·행동 1건 = 트랜잭션 1개. 판정(조회)·실행(쓰기) 어느 단계의 예외도 봇/행동 단위로 격리해 다음 행동·다음 봇으로 진행한다(전체 틱 중단 없음). 기존 서비스의 비즈니스 거절(이미 완료·거미줄 없음·revision 충돌 등)은 장애가 아닌 정상 경합으로 흡수한다.
  - **운영 전제**: 멀티 인스턴스 미지원(단일 인스턴스 전제, 다른 user-api 스케줄러와 동일 한계). 분산 락·활동 전용 스위치는 후속 → [open-questions.md](../../open-questions.md) "동거 봇".
  - 행동별 규칙: 루틴 완료 → [routine-todo/features.md](../routine-todo/features.md) "루틴 완료 처리" · 미션 기여·응원·방명록 → [house/features.md](../house/features.md) "동거 봇 활동" · 거미줄 청소·레이아웃 순환 → [room/features.md](../room/features.md).
