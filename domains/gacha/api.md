...
공통 규칙(prefix `/api/v1`, JSON, ISO-8601+offset, 이미지/에셋은 `*_key`, 목록은 `items` 배열, 재화는 `user_wallets.currency_type`, 인증 MVP 유예·소유권 식별자 유지)은 상위 [api.md](../../api.md)를 따른다. 아래 요청/응답 상세는 **설계 중(draft)** 이며 확정 아님.
...
- **요청 핵심 필드**: (path `id`) 머신 식별. 본문은 dev/seed user 기준 — 소유권 식별자 `userId`는 유지(최종 형태에서 `me` path 가능). 추가 옵션 **미정**(예: 연속 뽑기 수).
