...
- 설정 단위는 개별 `NotificationType`이 아니라 **설정 그룹**(`NotificationSettingType`)이다: `ALL`(전체 마스터) / `REMINDER`(리마인더) / `HOUSE`(집 알림). 그룹 매핑은 `REMINDER` ← `ROUTINE_REMINDER`·`TODO_REMINDER`·`WEEKLY_REPORT`·`DAILY_INCOMPLETE_DIGEST`, `HOUSE` ← `HOUSE_KICK`·`FRIEND_CHEER`·`HOUSE_MISSION_ACHIEVED`·`HOUSE_MEMBER_JOINED`·`HOUSE_MEMBER_LEFT`·`HOUSE_JOIN_REQUEST_CREATED`·`HOUSE_JOIN_REQUEST_ACCEPTED`·`HOUSE_JOIN_REQUEST_REJECTED`·`ROOM_COBWEB_CLEANED`.
...
- 발송 게이트는 push가 나가는 **모든 경로**에 적용된다. 공용 진입점 `NotificationService.send(...)`의 push 경로뿐 아니라, 이 진입점을 거치지 않고 batch worker가 직접 발송하는 리마인드(`ROUTINE_REMINDER`·`TODO_REMINDER`)·주간 회고 도착(`WEEKLY_REPORT`)·저녁 미완료 다이제스트(`DAILY_INCOMPLETE_DIGEST`)도 발송 직전에 같은 판정을 거친다. 즉 `REMINDER` 그룹 off와 마스터 `all` off는 리마인드·주간 회고·저녁 미완료 다이제스트 push에도 그대로 동작한다.
...
- 설정으로 차단된 리마인드·다이제스트도 알림 내역(`notification`) 저장은 그대로이고 `push_status`만 `BLOCKED`로 종결된다(FCM 호출 없음). 설정을 꺼도 알림함 목록에는 남는다.

## FCM 발송 인프라 (내부, 신규 엔드포인트 없음)

공용 진입점 `NotificationService.send(userId, content[, refId])` — `content`는 `NotificationContent`(type·title·body) 레코드이고 문구는 `NotificationMessages` 팩토리가 소유한다. 알림 내역을 `notification` 테이블에 호출 트랜잭션에서 저장(동기)하고, push 발송은 AFTER_COMMIT 이벤트 리스너가 트랜잭션 밖(`NOT_SUPPORTED`)에서 비동기로 트리거한다 — 내역은 항상 남고 push는 best-effort. `refId`는 발송 원인 리소스 ID(예: 리마인드면 routineId, 저녁 미완료 다이제스트면 digest id, 입주·퇴거면 membershipId)로 중복 발송 판정에 쓰며, 생략하면 null.

- `type`(`NotificationType`): `HOUSE_KICK`, `ROUTINE_REMINDER`, `TODO_REMINDER`, `FRIEND_CHEER`, `HOUSE_MISSION_ACHIEVED`, `HOUSE_MEMBER_JOINED`, `HOUSE_MEMBER_LEFT`, `HOUSE_JOIN_REQUEST_CREATED`, `HOUSE_JOIN_REQUEST_ACCEPTED`, `HOUSE_JOIN_REQUEST_REJECTED`, `ROOM_COBWEB_CLEANED`, `WEEKLY_REPORT`, `DAILY_INCOMPLETE_DIGEST`. 루틴 리마인드(`ROUTINE_REMINDER`)·투두 리마인드(`TODO_REMINDER`) 발송 트리거는 공용 batch worker(`reminderJob`)로 구현됨(5분 주기, 같은 분 재실행은 중복 발송 방지로 스킵; 상세는 아래 스케줄러 절). 집 멤버 응원(`FRIEND_CHEER`)은 응원 API(house 도메인)가 진입점 `send(...)`를 응원 저장과 같은 트랜잭션에서 직접 호출한다(`refId` = cheerId — 내역 저장은 동기, push는 위 AFTER_COMMIT 경로로 비동기). 단체 미션 달성(`HOUSE_MISSION_ACHIEVED`)은 미션 기여 공용부(`HouseMissionService.recordContribution` — 기여 API와 **연동 루틴 완료의 자동 기여 양쪽**)가 미션 행 락 트랜잭션 안에서 진입점 `send(...)`를 직접 호출한다 — 합산이 이번 기여로 처음 목표치에 도달한 순간(직전 합산 < target && 이후 ≥ target) 1회, 집 활성 멤버 전원(마지막 기여자 본인 포함)에게 발송한다. **WEEKLY 미션 한정** — DAILY 미션의 일일 달성 분기는 이 알림을 보내지 않는다. `claim`(보상 수령) 시점에는 발송하지 않는다. `refId` = missionId. 문구: 제목 "단체 미션 달성!" / 본문 "'{미션명}' 미션이 목표를 달성했어요. 보상을 받아보세요!". 중복 방지는 별도 dedupe 쿼리 없이 미션 행 락 하의 "처음 도달" 조건 판정으로 충분하다. 입주(`HOUSE_MEMBER_JOINED`)·퇴거(`HOUSE_MEMBER_LEFT`)는 가입 확정(초대코드·신청 수락 공통 경로)·탈퇴 트랜잭션에서 나머지 ACTIVE 멤버 전원에게 발송한다 — `refId` = 해당 membership id. 문구: "새 멤버 입주" / "{닉네임}님이 집에 입주했어요.", "멤버 퇴거" / "{닉네임}님이 집을 떠났어요.". 신규 엔드포인트 없음. 입주 신청 수명주기 알림 3종: 신청 도착(`HOUSE_JOIN_REQUEST_CREATED`)은 신청 생성·재오픈(탐색 신청·구성원 개인 코드 참여 공통) 트랜잭션에서 **방장에게** 발송한다 — 문구: "입주 신청 도착" / "{신청자 닉네임}님이 '{집 이름}'에 입주를 신청했어요. 수락하면 입주가 확정돼요."(닉네임 null 이면 "이웃"으로 폴백). 같은 수신자·타입·본문 조합은 **1시간 안에 재발송하지 않는다** — 철회→재신청(신청 행이 새로 생겨 `refId`가 바뀜)·거절→재신청(reopen)을 반복해 방장에게 push 를 무제한 밀어넣는 증폭 방어이며, 억제되는 것은 알림뿐이고 신청 자체는 생성된다. `refId` = requestId 인데 신청자가 철회하면 대상 신청 행이 하드 삭제되므로 **알림의 refId 가 삭제된 신청을 가리킬 수 있다** — 프론트는 신청 목록 화면 폴백을 두어야 한다. 수락(`HOUSE_JOIN_REQUEST_ACCEPTED` — "입주 신청 승인" / "'{집 이름}' 입주 신청이 승인됐어요.")·거절(`HOUSE_JOIN_REQUEST_REJECTED` — "입주 신청 거절" / "'{집 이름}' 입주 신청이 거절됐어요.")은 각 처리 트랜잭션에서 **신청자 본인에게** 발송한다(`refId` = requestId). 방 거미줄 청소(`ROOM_COBWEB_CLEANED`) 트리거·문구는 [house/api.md](../house/api.md) 거미줄 청소 항목 참고. 강퇴(`HOUSE_KICK`) 알림 트리거는 미구현(후속 — enum 값만 존재). 주간 회고 도착(`WEEKLY_REPORT`)은 주간 배치(`weeklyReportPushJob`)가 발송한다 — 회고 생성 배치(일요일 00:30 KST)와 **분리된 발송 시각**(일요일 20:00 KST) 이후 매시 트리거가 가장 최근 끝난 주의 회고에 대해 알림을 적재·발송하며(리마인드와 같은 batch 발송 경로 재사용, 진입점 미경유), `refId` = 주간 회고 id로 중복 발송을 막는다. 대상은 항상 가장 최근 끝난 주뿐이라 서버가 오래 멈췄다 살아나도 지난 주들의 뒤늦은 push는 나가지 않는다. 문구: 제목 "지난주 루틴 회고가 도착했어요" / 본문 "지난주 루틴 {N}회 중 {M}회를 완료했어요. 이번 주 계획 전에 확인해 보세요."(서버 집계 수치만 사용, LLM 문구 미사용). 저녁 미완료 다이제스트(`DAILY_INCOMPLETE_DIGEST`)는 아래 "저녁 미완료 다이제스트" 절의 스냅숏을 기준으로 발송하며 `refId` = `daily_incomplete_digests.id`다. `WEEKLY_REPORT`와 `DAILY_INCOMPLETE_DIGEST`의 설정 그룹은 `REMINDER`.
...
## 저녁 미완료 다이제스트 (내부, 신규 사용자 엔드포인트 없음)

매일 저녁 남은 일을 한 번 묶어 알려준다. `DAILY_INCOMPLETE_DIGEST`는 `REMINDER` 그룹 알림이며, 사용자 설정으로 push가 차단되어도 다이제스트와 알림 내역은 저장한다.

- **발송 시점**: 매일 21:00 KST 이후 현재 KST 날짜만 대상으로 한다. 트리거는 매시 정각(KST) + 앱 기동 시 1회 catch-up이며, 오늘(`digestDate`) 다이제스트가 이미 생성되어 있으면 no-op이다. 발송 reader와 실제 push 직전 writer에서 현재 KST 날짜를 다시 확인하므로 23:59에 시작한 job도 자정을 넘기면 전날 알림을 보내지 않는다. 서버가 늦게 살아나도 어제 이전 날짜는 소급 발송하지 않는다.
- **대상 사용자**: 탈퇴 사용자와 봇 계정은 제외한다. 아래 대상 항목 합계가 0개인 사용자는 다이제스트를 만들지 않는다.
- **대상 루틴**: 오늘 반복 대상인 `ACTIVE`·미삭제 루틴 중, 같은 계보(`origin_routine_id`)에 오늘(`routine_logs.routine_date = digestDate`) `COMPLETED` 로그가 없는 루틴. 대상 스냅숏에는 루틴 row id가 아니라 계보 id를 `ROUTINE` target으로 저장한다.
- **대상 투두**: `due_date = digestDate`, `status = PENDING`, 미삭제 투두. `due_time` 유무와 무관하게 오늘 마감 미완료면 포함한다. 대상 스냅숏에는 투두 id를 `TODO` target으로 저장한다.
- **중복 방지와 스냅숏**: `daily_incomplete_digests`는 사용자·날짜당 1건(`UNIQUE(user_id, digest_date)`)이며, 생성 시점의 `routine_count`, `todo_count`와 대상 항목을 `daily_incomplete_digest_targets`에 함께 저장한다. 전체 수는 두 count의 합으로 계산하며, 이후 사용자가 항목을 완료해도 스냅숏 row는 바꾸지 않는다.
- **알림 연결**: 다이제스트 생성 트랜잭션에서 `notification.type = DAILY_INCOMPLETE_DIGEST` 알림을 만들고 `notification.ref_id = daily_incomplete_digests.id`로 연결한다. `daily_incomplete_digests.notification_id`도 같은 알림을 가리킨다.
- **문구**: 제목은 "오늘의 루틴을 마무리해 볼까요?", 본문은 `오늘 아직 {total}개가 남았어요. 루틴 {r}개 · 투두 {t}개를 마무리해볼까요?`를 사용한다. `{total}`은 `routine_count + todo_count`, `{r}`은 `routine_count`, `{t}`는 `todo_count`.
- **상태**: 저장 직후 `notification.push_status`와 `daily_incomplete_digests.push_status`는 `PENDING`이고, push 결과에 따라 `SENT`/`BLOCKED`/`FAILED`로 같은 상태를 기록한다. `sent_at`은 실제 push 성공 시각을 저장하며 완료 전환창도 이 시각을 기준으로 계산한다.

## 저녁 미완료 다이제스트 관리자 지표

| method · path | 목적 | 핵심 필드 | 관련 table |
| --- | --- | --- | --- |
| `GET /admin/notification-digests/metrics?days=N` | 최근 N일 저녁 미완료 다이제스트 생성·발송·전환 지표 조회 | query: `days`(기본 7·최대 90) / resp: `days[]`(`date`, `createdCount`, `pendingCount`, `sentCount`, `blockedCount`, `failedCount`, `sentRate`, `conversionMeasuredCount`, `convertedCount`, `conversionPendingCount`, `conversionRate`), `generatedAt` | `daily_incomplete_digests`, `daily_incomplete_digest_targets`, `notification` |

- 집계 단위는 KST 날짜(`digest_date`)이고, 최근 N일에는 오늘을 포함한다.
- 상태별 수는 `PENDING`/`SENT`/`BLOCKED`/`FAILED`를 모두 분리한다. `PENDING`은 아직 발송 결과가 확정되지 않은 건이다.
- 전환은 `SENT`인 다이제스트만 분모로 삼는다. `BLOCKED`/`FAILED`/`PENDING`은 전환율 분모에서 제외한다.
- 전환 성공은 실제 발송 시각(`sent_at`)부터 2시간 미만에 스냅숏 대상 중 하나 이상이 완료된 경우다. 발송 전 완료와 정확히 120분 경계 이후 완료는 제외하며, `ROUTINE` target은 같은 계보의 오늘 `COMPLETED` 로그, `TODO` target은 해당 투두의 `COMPLETED` 상태와 `completed_at`으로 판정한다.
- 2시간 전환창이 아직 끝나지 않은 `SENT` 건은 `conversionPendingCount`로 따로 세고 `conversionRate` 분모에서는 제외한다. 따라서 `conversionMeasuredCount`와 전환율 분모는 전환창이 닫힌 `SENT` 건이다.
- 운영 WATCH: MVP 구현은 최대 90일 digest를 조회해 애플리케이션에서 일별 상태를 집계한다. 사용자 규모가 커지기 전에 DB `group by` 집계와 target 전환 조회의 실행 계획·인덱스를 점검한다.
