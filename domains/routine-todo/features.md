...
- **수행 시간 / 기간**: `scheduled_time`(TIME), `starts_on`·`ends_on`(DATE)으로 시간순 정렬 및 노출 기간을 정한다. `starts_on`은 **미지정 시 생성일(오늘, KST)로 기본 지정**하며, **오늘 이전 과거로는 설정할 수 없다**(루틴은 생성일부터 존재하는 스냅숏 구조). `starts_on`은 `ends_on`보다 늦을 수 없다.
