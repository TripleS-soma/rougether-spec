...
- **동거 봇의 루틴 완료**: 동거 봇 계정(`users.is_bot`, [member/features.md](../member/features.md) "동거 봇 계정")의 루틴은 활동 스케줄러가 완료한다 — 봇 루틴마다 하루 완료 확률 **70%**(쉬는 날 15%에는 전부 스킵), 완료하는 날이면 봇의 활동 창 안 목표 틱 이후 첫 틱에 위 완료 체크 경로를 그대로 호출한다. 그래서 코인 10·일일 보상 상한 50·스트릭·같은 날짜 중복 완료 거부가 사람과 동일하게 적용된다. 봇은 **당일 완료만** 하며 과거 날짜 완료·완료 취소는 하지 않는다. 사람 ACTIVE 구성원이 있는 집에 속한 봇만 대상이고, 봇의 완료 로그는 일반 회원과 같이 기록·노출된다.
