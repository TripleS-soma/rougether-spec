...
- **수행 시간 / 기간**: `scheduled_time`(TIME), `starts_on`·`ends_on`(DATE)으로 시간순 정렬 및 노출 기간을 정한다. `scheduled_time`은 **5분 단위만 사용**한다(앱 입력 제한, 서버 검증 없음) — 리마인드 배치가 5분 주기 + 분 정확 일치 매칭이라 그 외 시각은 리마인드가 발송되지 않는다(notification 도메인 참고). `starts_on`은 **미지정 시 생성일(오늘, KST)로 기본 지정**하며, **오늘 이전 과거로는 설정할 수 없다**(루틴은 생성일부터 존재하는 스냅숏 구조). `starts_on`은 `ends_on`보다 늦을 수 없다.
...
- **늦은 완료·취소**: `FAILED` 로그가 있는 날짜의 완료는 새 row 생성 대신 그 로그를 `COMPLETED`로 전이한다(과거 완료 규칙 그대로 보상 0·스트릭 미반영). 전이된 완료의 취소는 기존 취소 규칙대로 hard delete 한다(`FAILED`로 되돌리지 않음). 배치는 지나간 날짜의 로그를 재생성하지 않으므로 그 날짜 기록은 사라진다(의도된 동작).
