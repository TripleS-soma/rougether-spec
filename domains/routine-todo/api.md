...
| method · path | 목적 | 요청 핵심 | 응답 핵심 |
| --- | --- | --- | --- |
| `POST /api/v1/routines/{id}/logs` | 완료 체크(과거 허용·미래 불가) · `status=SKIPPED`면 발생분 건너뜀(오늘·미래만) | `routineDate`(기본 오늘), `status?`(`COMPLETED` 기본 / `SKIPPED`) | 생성된 log: `id`, `routineDate`, `status`, `completedAt`, `rewardCurrencyType`, `rewardAmount` + streak 요약 + `houseMissionContribution?`(연동 단체미션 자동 기여 결과 — 미연동·기여 건너뜀이면 null. 규칙은 [house api.md](../house/api.md) contribute 참고) |
| `DELETE /api/v1/routines/{id}/logs` | 완료 취소(과거 허용·미래 불가) · 그 날짜에 `SKIPPED`가 있으면 건너뜀 해제(미래 허용) | `date`(취소할 완료/건너뜀 날짜, query) | 롤백 결과(반영된 streak 요약). 트랜잭션 처리 |
...
> 응답 `status` 허용값은 `PENDING`/`COMPLETED`/`FAILED`(하루 마감 배치가 전날 미수행 루틴에 기록)/`SKIPPED`(발생분 건너뜀).
> **발생분 건너뜀 (`status=SKIPPED`, 2026-09-13, mobile #189)** — 반복 규칙은 그대로 두고 **그 날짜 한 번만** 수행 대상에서 뺀다(앱의 "이 날 몫을 다른 날로 옮기기"). 오늘·미래 날짜만(지난 날짜는 400 `SKIP_DATE_NOT_ALLOWED` — 이미 완료/실패로 판정이 끝났음), 살아있는 현재 버전 id만. 보상·스트릭·연동 단체미션 기여 **없음**, 지갑 불변. `routine_logs`에 `SKIPPED` row 하나(`completed_at` null, 보상 0)를 남기며 unique(`routine_id`, `routine_date`)를 점유하므로 day-end 배치가 그날 `FAILED`를 만들지 않는다. 같은 날짜 재요청은 기존 row를 그대로 돌려준다(멱등, 201). 이미 `COMPLETED`인 날짜는 409 `ALREADY_COMPLETED`. **해제**는 `DELETE …/logs?date=`(row 삭제, 미래 날짜 허용, 스트릭 요약은 그대로). 건너뛴 날짜를 **다시 완료**하면 `SKIPPED` row를 지우고 일반 완료 규칙(당일 보상·스트릭)을 그대로 적용한다. `/today`·`/calendar`·`/calendar/month`·리마인드 배치는 `SKIPPED` 발생분을 **계보 단위로 제외**한다(아래 소싱 규칙). 과거 날짜 완료 시 그 날짜에 `FAILED` 로그가 있으면 새 row를 만들지 않고 그 row를 `COMPLETED`로 **전이(UPDATE)** 한다 — 응답 `id`는 기존 row의 id이고, 보상 0·스트릭 미반영은 과거 완료 규칙 그대로. 전이된 완료의 취소는 위 취소 규칙에 따라 다시 `FAILED`로 복원된다. 배치는 지나간 날짜의 로그를 재생성하지 않는다.
...
> - **날짜별 소싱 규칙은 `/calendar`와 동일하다.** 루틴은 아래 3갈래 규칙(오늘·미래는 현재 ACTIVE 버전의 반복 대상에서 `SKIPPED` 계보 제외, 어제는 그날 유효했던 버전으로 재계산(`SKIPPED` 계보 제외), 그제 이전은 그날 `routine_logs`(`COMPLETED`+`FAILED`, `SKIPPED` 제외) 건수 — 로그 없는 날은 0), 투두는 `dueDate`가 그날인 것만(마감일 없는 투두 제외). 따라서 어떤 날짜의 개수는 그 날짜를 `/calendar`로 조회했을 때의 루틴·투두 건수와 항상 같다.
...
> - **오늘·미래(`date >= 오늘 KST`)**: 그 날짜의 반복 대상 루틴을 현재 ACTIVE 버전으로 live 재계산해 노출하고, 완료 여부는 그 날짜 `routine_logs`(`routine_date`)로 판정한다. 그 날짜에 `SKIPPED` 로그가 있는 **계보**(`coalesce(origin_routine_id, id)`)는 노출하지 않는다(건너뜀 뒤 버전이 분기돼도 숨김 유지).
