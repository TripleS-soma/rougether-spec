...
- **동거 봇의 청소**: 같은 집 사람 방에 활성 거미줄이 있으면 동거 봇이 활동 창(프로필 시간대) 안 **10분 틱마다 10% 확률**로 위 청소 경로(같은 집 구성원 청소)를 그대로 호출한다. 청소 보상 3코인은 봇 지갑에 들어가고, 방 주인에게는 기존과 같이 `ROOM_COBWEB_CLEANED` 알림이 간다. 사람이 이미 청소했으면 봇의 시도는 정상 거절로 흡수된다. 스케줄러 공통 규칙은 [member/features.md](../member/features.md) "동거 봇 계정".
