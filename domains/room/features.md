...
- **방 레벨 조회**: `growth_level`은 0에서 시작하고 최소 누적 포인트 `T(L) = L × (L + 19)`를 충족한 가장 높은 레벨 L로 계산한다. 다음 구간 필요량은 `20 + 2 × L`로 20 → 22 → 24 → …씩 점진적으로 늘어난다. 누적 포인트와 다음 레벨까지 남은 포인트를 함께 보여준다.
- **성장 적립**: 루틴·투두 완료에서 실제 지급한 코인 보상과 같은 양의 성장 포인트를 별도로 적립한다. 현재 둘 다 기본 10포인트이며 루틴·투두 합산 하루 최대 50포인트다. 코인 부분 지급이면 포인트도 같은 양만 적립하고, 과거 날짜·투두 마감일 없음·일일 한도 소진·FAILED 이력 소급 완료처럼 코인이 0이면 포인트도 0이다. 코인 소비·다른 보상(가입·출석·초대·거미줄 등)은 성장에 영향을 주지 않는다.
- **완료 취소**: 해당 완료 건에 기록한 `growth_reward_amount`만 회수하고 레벨을 재계산한다. 취소로 레벨이 낮아질 수 있으며, 취소 후 재완료는 그때의 보상 한도에 따라 다시 적립한다. 완료 상태·코인·성장은 같은 트랜잭션에서 처리한다. 루틴/투두 자체의 삭제는 기존 완료 보상 유지 정책을 따르며, 완료 취소와 구분한다.
- **기존 데이터**: 기존 `growth_level`은 해당 레벨의 최소 누적 포인트로 보존한다. 활성 일반 회원의 도입 전 `COMPLETED` 루틴·투두 이력에도 실제 `COIN` 보상액의 미적립 차액을 일괄 반영하고 `growth_reward_amount`를 함께 기록한다. 보상액이 보존된 soft delete 이력도 포함하며 봇·탈퇴 회원·보상 0·다른 통화는 제외한다. 소급 후 완료 취소 시 실제 적립량을 회수한다. 코인 잔액과 배치를 바꾸지 않으며 재실행해도 중복 적립하지 않는다.
- **레벨 5 보상 — 모루**: 누적 120포인트로 5 이상을 처음 달성하면 `moru` 캐릭터를 1회 지급한다. 최고 달성 레벨 `highest_growth_level`을 보존하고 완료 취소로 레벨이 내려가도 보유를 유지한다. 한 번에 5를 넘거나 동시 완료·재시도가 발생해도 중복 지급하지 않으며, 기존 대표 캐릭터는 변경하지 않는다. 삭제된 지급 이력도 자동 재지급하지 않는다.
- **보상 지급 보정**: 기존 5 이상 사용자와 모루 미등록/비활성 기간의 달성자는 내 방·보유 캐릭터 목록 조회 때 보정한다. 에셋 미준비로 루틴/투두 완료를 실패시키지 않는다. 타인의 조회는 보상을 지급하지 않는다. 레벨 5 보상은 사용자 확정이며, 보유 유지·조회 시 보정·대표 유지는 이번 서버 구현 정책이다.
- **레벨업 피드백**: 서버는 갱신된 레벨·포인트를 제공한다. 모바일 연출과 레벨별 아이템 보상은 이번 범위에 포함하지 않는다.
