...
- `character`: `{ characterId, code, name, assetKey, animations, accessories[] }`. `accessories[]`는 `{ userItemId, itemId, name, assetKey, characterSlotType, equippedAt }`이며 슬롯·보유 아이템 순으로 정렬한다. 대표 캐릭터가 없으면 `character=null`, 악세사리가 없으면 빈 배열이다.
...
- 캐릭터가 포함되는 친구 방도 같은 `character.accessories[]` 계약을 사용한다. 비구성원에게 공개되는 집 멤버 방/미리보기의 `accessories[]`는 렌더링에 필요한 `{ itemId, name, assetKey, characterSlotType }`만 내려주고 소유 식별자 `userItemId`와 적용 시각 `equippedAt`은 제외한다.
- table: `personal_rooms`(읽기), `room_surface_slots`·`room_item_placements`(읽기), `user_items`/`items`(assetKey 조인), `user_character_accessories`(대표 캐릭터 착용 조회), `streaks`(읽기 전용).
