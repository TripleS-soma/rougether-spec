...
- table: `personal_rooms`(읽기), `room_surface_slots`·`room_item_placements`(읽기), `user_items`/`items`(assetKey 조인), `user_character_accessories`(대표 캐릭터 착용 조회), `character_accessory_render_profiles`(캐릭터별 합성 정보), `streaks`(읽기 전용).
