...
### `POST /api/v1/rooms/me/cobweb/clean`
- 목적: 내 방의 활성 거미줄을 청소하고 소액 보상을 받는다.
- res: `roomUserId`, `cleanedAt`, `rewardCurrencyType`(`COIN`), `rewardAmount`(MVP 3), `balance`.
- 동시성: 거미줄 행과 청소자 코인 지갑을 잠가 최초 한 요청만 성공·지급한다.
- 예외: 활성 거미줄 없음 `ROOM_COBWEB_NOT_ACTIVE`(409), 코인 지갑 없음 `ROOM_COBWEB_REWARD_WALLET_NOT_FOUND`(404).
- table: `room_cobwebs`, `user_wallets`, `wallet_histories`.
